...
public class DateUtil {
    ...
    static {
        DateUtil.SDF.setTimeZone(TimeZone.getTimeZone("UTC"));
    }
    ...
    public static String timeToString(Date time, final int style) {
        ...
        switch (style) {
            case 1: {
                timeStyle = DateUtil.TIME_STYLE_S1;
                break;
            }
            case 2: {
                timeStyle = DateUtil.TIME_STYLE_S2;
                break;
            }
            case 3: {
                timeStyle = DateUtil.TIME_STYLE_S3;
                break;
            }
            case 4: {
                timeStyle = DateUtil.TIME_STYLE_S4;
                break;
            }
            case 5: {
                timeStyle = DateUtil.TIME_STYLE_S5;
                break;
            }
            case 6: {
                timeStyle = DateUtil.TIME_STYLE_S6;
                break;
            }
            case 7: {
                timeStyle = DateUtil.TIME_STYLE_S7;
                break;
            }
            case 8: {
                final SimpleDateFormat sdf = (SimpleDateFormat) DateUtil.SDF.clone();
                return sdf.format(time);
            }
            case 9: {
                return DateUtil.getEpochTime(time);
            }
            default: {
    ...
    public static String timeToStringNull(final Date time, final int style) {
        return time == null ? null : DateUtil.timeToString(time, style);
    }
    ...
    public static Date parseUTCTime(final String utcTime) throws ParseException {
        ...
        final SimpleDateFormat sdfi = (SimpleDateFormat) DateUtil.SDF.clone();
